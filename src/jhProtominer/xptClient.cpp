...
{
	uint32 usernameLength = min(127, strlen(xptClient->username));
	uint32 passwordLength = min(127, strlen(xptClient->password));
	// build the packet
	bool sendError = false;
	xptPacketbuffer_beginWritePacket(xptClient->sendBuffer, XPT_OPC_C_AUTH_REQ);
	xptPacketbuffer_writeU32(xptClient->sendBuffer, &sendError, 5);								// version (4 -> new reward system and improved worker details)
	...
	xptPacketbuffer_writeString(xptClient->sendBuffer, minerVersionString, 68, &sendError);		// minerVersionString
	// finalize
